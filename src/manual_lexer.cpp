...
vector<Token> tokenize(const string &src) {
    ...
    while (pos < src.size()) {
        ...
        if (pos + 1 < src.size() && src[pos] == '/' && src[pos + 1] == '*') {
            ...
            if (pos + 1 >= src.size()) {
                cerr << "Error: Unclosed multi-line comment at line " 
                     << start_line << ", column " << start_column << endl;
                break;
            }
            pos += 2; column += 2;
            continue;
        ...
        if (c == '"') {
            ...
            while (pos < src.size() && (escaped || src[pos] != '"')) {
                ...
                escaped = (src[pos] == '\\' && !escaped);
                value += src[pos++];
            }
            if (pos >= src.size()) {
                cerr << "Error: Unclosed string literal at line " 
                     << start_line << ", column " << start_column << endl;
                break;
            }
            value += '"'; pos++; column++;
        ...
        if (isalpha(c) || c == '_') {
            ...
            if (invalid) {
                cerr << "Error: Invalid identifier '" << value 
                     << "' at line " << start_line << ", column " << start_column << endl;
            ...
            if (c == '0' && pos + 1 < src.size() && 
               (src[pos + 1] == 'x' || src[pos + 1] == 'X')) {
                ...
                while (pos < src.size() && (isdigit(src[pos]) || 
                      (tolower(src[pos]) >= 'a' && tolower(src[pos]) <= 'f'))) {
                    value += src[pos++]; column++;
                }
        ...
        if (pos + 1 < src.size()) {
            ...
            if (two_chars == "==") { type = T_EQUALSOP; value = two_chars; pos += 2; column += 2; }
                ...
                switch (c) {
                    ...
                    default:
                        cerr << "Error: Unknown token at line " << line 
                             << ", column " << column << " -> '" << c << "'" << endl;
                        break;
                }
                pos++; column++;
            }
...
vector<Token> ManualLexer::tokenize() {
